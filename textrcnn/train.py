...
from textrcnn.processdata import Dataset
from textrcnn.textrncctest import RCNN
...
def nextBatch(x, y, batchSize):
    ...
    numBatches = int(len(perm)/batchSize)
    print("*******************************")
    if numBatches == 0:
        numBatches +=1
    elif numBatches % batchSize > 0:
        numBatches +=1
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        for i in range(config.training.epoches):
            ...
            batchs = nextBatch(trainReviews, trainLabels, config.batchSize)
            print(batchs)
            for batchTrain in batchs :
                print("in here")
